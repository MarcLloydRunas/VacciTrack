...
if(isset($_POST["submitrequest"])) {
    ...
    if(empty($site_err) && empty($fname_err) && empty($username_err) && empty($lname_err) && empty($mname_err) && empty($contact_err) && (in_array($extension, ['png', 'jpg', 'jpeg', 'webp']))) {
        ...
        if($stmt = $pdo->prepare($sql)){
            ...
            if($stmt->execute()){
                ...
                echo "<div class='container aligns-items-center justify-content-center' style='width:500px; position:absolute; top:-20px; left: 40%'>
                            <div class='container alert alert-success alert-dismissible d-flex align-items-center fade show' style='margin:-50px;'>
                                <i class='bi-check-circle-fill'></i>
                                    <strong class='mx-1'>Success!</strong> Request Sent.
                ...
                try {
                    ...
                    $mail->SMTPAuth = true;                         
                    $mail->Username = 'vaccitrack01';             
                    $mail->Password = 'tneqkmknokhbxakf';                       
                    $mail->SMTPSecure = 'tls';                          
                    $mail->Port  = 587;

                    $mail->setFrom('vaccitrack01@gmail.com', 'VacciTrack');       
                    $mail->addAddress('vaccitrack01@gmail.com');
        ...
        echo "<div class='container aligns-items-center justify-content-center' style='width:500px; position:absolute; top:-10px; left:0; right:0'>
                                    <div class='container alert alert-danger alert-dismissible d-flex align-items-center fade show' style='margin:-50px;'>
                                            ...
                                            <strong class='mx-1'>Error!</strong> Image format not supported. Please try again.
                                            <button type='button' class='btn-close' data-bs-dismiss='alert'></button>
                                    </div>
                              </div>";
    }
...
<div class="container-fluid" id="veri-con">
    ...
    <form name="submitrequestform" enctype="multipart/form-data" method="POST" action="">
        <div class="row">
        ...
        <div>
            <hr>
            <input class="form-control btn btn-primary" type="submit" name="submitrequest" value="submit">
        </div>
        <?php 
            if(!empty($login_err)){
